...
use si\helpers\Session;
    ...
    class APIIntegrada {
        ...
        const URI = 'www.sistemaintegrado.com.br/si';
        const MODULE = 'api';
         ...
         * @return string
         */
        public static function getURI($incModuleApi = true)
        {
            $uri = self::getProtocol() . '://';

            if (self::$IS_TEST and strpos($_SERVER['HTTP_HOST'], 'localhost:8090') !== false) {
                $uri .= 'localhost:8090/sistemaintegrado.com.br';
            } else {
                $uri .= self::URI;
            }
        ...
        private function curl($action, $values = null, $toObject = true, $decodeJson = true)
        {
            ...
            # CURL
            $ch = curl_init(self::getURI() . "/{$action}");
